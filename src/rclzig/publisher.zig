...
pub const PublisherOptions = struct {
    ...
    pub fn deinit(self: *PublisherOptions) void {
        const fini_ret = rcl.rcl_publisher_options_fini(&self.rcl_options);
        if (fini_ret != rcl.RCL_RET_OK) {
            std.log.err("failed to finalize rcl_publisher_options_t ({})\n", .{fini_ret});
        }
    }
};
...
pub fn Publisher(comptime MsgType: type) type {
    return struct {
        ...
        pub fn init(node: Node, topic_name: []const u8, options: PublisherOptions) !Self {
            ...
            const init_ret = rcl.rcl_publisher_init(&publisher.rcl_publisher, &node.rcl_node, &publisher.type_support.rcl_type_support, @ptrToInt(topic_name.ptr), &options.rcl_options);
        ...
        pub fn deinit(self: *Publisher, node: Node) void {
...
//     var publisher = Publisher(std_msgs.msg.String).init(node, "chatter", publisher_options);
